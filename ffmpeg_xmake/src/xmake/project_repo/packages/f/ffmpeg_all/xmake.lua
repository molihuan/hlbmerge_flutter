package("ffmpeg_all")
    ...
    on_load(function (package)
        ...
        if package:config("pic") ~= false and package:is_plat("linux", "android" ,"harmony") then
    ...
    -- oh add
    on_install("windows", "mingw@windows,linux,cygwin,msys", "cross", "linux", "macosx", "android", "iphoneos","harmony","wasm", function (package)
        ...
        elseif package:is_plat("wasm") then
            ...
            table.insert(configs, "--disable-programs")
        else
        ...
        elseif package:is_plat("android") then
            ...
            for i, config in ipairs(configs) do
                print(string.format("  [%d] %s", i, config))
            end
        ...
        elseif package:is_plat("harmony") then
            ...
            print("开始:")
            print("get_config:" .. get_config("sdk"))
            local native_dir = get_config("sdk")
            local bin = path.join(native_dir, "/llvm/bin")
            ...
            if package:is_arch("arm64-v8a") then
                ...
                cross_prefix = path.join(bin, "aarch64-unknown-linux-ohos-")
            elseif package:arch():startswith("armeabi-v7a") then
                arch = "arm"
                cpu = "armv7-a"
                triple = "armv7-unknown-linux-ohos"
                cross_prefix = path.join(bin, "armv7-unknown-linux-ohos-")
            elseif package:is_arch("x86_64") then
                arch = "x86_64"
                cpu = "x86_64"
                triple = "x86_64-unknown-linux-ohos"
                cross_prefix = path.join(bin, "x86_64-unknown-linux-ohos-")
            else
                raise("unknown arch(%s) for harmony!", package:arch())
            end
            local sysroot = path.join(native_dir, "sysroot")
            local cflags   = table.join(table.wrap(package:config("cxflags")), table.wrap(package:config("cflags")), table.wrap(get_config("cxflags")), get_config("cflags"))
            -- 加 OHOS 宏
            table.insert(cflags, "-DOHOS_NDK")
            table.insert(cflags, "-D__MUSL__=1")
            table.insert(cflags, "-fPIC")
            table.insert(cflags, "-O3")
            table.insert(cflags, "-fstack-protector-strong")

            local cxxflags = table.join(table.wrap(package:config("cxflags")), table.wrap(package:config("cxxflags")), table.wrap(get_config("cxflags")), get_config("cxxflags"))

            -- 加 OHOS 宏
            table.insert(cxxflags, "-DOHOS_NDK")
            table.insert(cxxflags, "-D__MUSL__=1")
            table.insert(cxxflags, "-fPIC")
            table.insert(cxxflags, "-O3")
            table.insert(cxxflags, "-fstack-protector-strong")

            local cc = path.join(bin, triple .. "-clang")
            local cxx = path.join(bin, triple .. "-clang++")
            local ar = path.join(bin, "llvm-ar")
            local ranlib = path.join(bin, "llvm-ranlib")
            local strip = path.join(bin, "llvm-strip")
            local ld = path.join(bin, "ld.lld")
            ...
            for i = #configs, 1, -1 do
                if configs[i]:startswith("--arch=") then
                    table.remove(configs, i)
                end
            end

            --table.insert(configs, "--disable-avdevice")
            table.insert(configs, "--disable-vulkan")
            -- 已经设置过了
            --table.insert(configs, "--enable-cross-compile")
            table.insert(configs, "--arch=" .. arch)
            -- 好像没有什么用
            ...
            table.insert(configs, "--ar=" .. path.unix(ar))
            table.insert(configs, "--ranlib=" .. path.unix(ranlib))
            table.insert(configs, "--strip=" .. path.unix(strip))
            table.insert(configs, "--ld=" .. path.unix(cc))
            if #cflags > 0 then
                table.insert(configs, "--extra-cflags=" .. table.concat(cflags, ' '))
            end
            if #cxxflags > 0 then
            ...
            -- 打印 argv
            print("Make 初始命令参数: " .. table.concat(argv, " "))
        ...
        elseif package:is_plat("wasm") then
            ...
            local emcc_sdk = get_config("sdk")
            print("get_config emcc_sdk:" .. emcc_sdk)

            local bin = path.join(emcc_sdk, "/upstream/emscripten")
            print("bin:" .. bin)

            local cc = path.join(bin, "emcc")
            local cxx = path.join(bin, "em++")
            local ar = path.join(bin, "emar")
            local ranlib = path.join(bin, "emranlib")
            local strip = path.join(bin, "emstrip")
            local nm = path.join(bin, "emnm")
            ...
            table.insert(configs, "--ranlib=" .. path.unix(ranlib))
            table.insert(configs, "--strip=" .. path.unix(strip))
            table.insert(configs, "--nm=" .. path.unix(nm))
            ...
            local make_exec = "make"

            os.vrunv(make_exec, argv)
            os.vrun(make_exec .. " install")
